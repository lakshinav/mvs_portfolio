...
% options=optimset(options, 'InitTrustRegionRadius',0.02);
...
%% Estimation (spatial CCC)
warning off all
type=4;
f=@(u) cmptLLspCcc(u,y,W,type);
lb=setBndsCcc(type,n);
b=@(v) checkSigmaPd(v,W,type);
k=6;
% vc(k,1).x0=[2e-5*ones(1,n) 0.1*ones(1,n) 0.1*ones(1,n) 0.1*ones(1,n) 0.02 0.01 0.03];
% vc(k,1).x0=[2e-5*ones(1,n) 0.1*ones(1,n) 0.1*ones(1,n) 0.1*ones(1,n) 0.02*ones(1,4) 0.01*ones(1,4) 0.03*ones(1,4)];
vc(k,1).x0=[2e-5*ones(1,n) 0.1*ones(1,n) 0.1*ones(1,n) 0.1*ones(1,n) 0.02*ones(1,n) 0.01*ones(1,n) 0.03*ones(1,n)];
tic;
[vc(k,1).x,vc(k,1).fv,vc(k,1).ef,vc(k,1).op]=fmincon(f,vc(k,1).x0,[],[],[],[],lb,[],b,options);
...
clear f b;
disp(vc(k).signif);
[~,sigma]=cmptLLspCcc(vc(k).x,y,W,type);
p=-1*ones(1,T);
for i=1:T
    [~,p(i)]=chol(sigma(:,:,i));
end
